...
function animateCards(swiper, reverse = false, zIndex = 1) {
  ...
  if (isDesktop) {
    timeline.fromTo(`${swiper} .service_tab-item.is-01`, 
      {
        x: reverse ? 20 : 150,
        rotate: -3
      },
    ...
    .fromTo(`${swiper} .service_tab-item.is-03`,
      {
        x: reverse ? -20 : -150,
        rotate: 3
      },
...
selectors.forEach(selector => {
  selector.addEventListener('click', () => {
    ...
    const currentActive = document.querySelector('.service_tab-selector.is-active');
    const currentContent = document.querySelector(`.swiper.service-tabs[tab-content="${currentActive.getAttribute('tab')}"]`);
    const newContent = document.querySelector(`.swiper.service-tabs[tab-content="${tabNumber}"]`);
    
    currentActive.classList.remove('is-active');
    selector.classList.add('is-active');
    
    animateCards(`.swiper.service-tabs[tab-content="${currentActive.getAttribute('tab')}"]`, true, 1);
...
const firstSwiper = document.querySelector('.swiper.service-tabs[tab-content="1"]');
